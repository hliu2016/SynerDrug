...
for (i in 1:k){
  ...
  prob<-predict(object =logisticmodel,test,type = "response")
  pred<-ifelse(prob>0.5,"1","0")
  #prediction <- predict(logisticmodel,test[,!names(test) %in% c("class")],probability=TRUE)
  prediction <- prediction(as.numeric(prob),as.numeric(test$class))
  perf <- performance(prediction,"tpr","fpr")
  auc <- performance(prediction,'auc')
  auc = slot(auc,"y.values")
  table(test$class, pred,dnn=c("true","pred"))
  TP<-as.numeric(table(test$class, pred,dnn=c("true","pred"))[2,2])
  TN<-as.numeric(table(test$class, pred,dnn=c("true","pred"))[1,1])
  FP<-as.numeric(table(test$class, pred,dnn=c("true","pred"))[1,2])
  FN<-as.numeric(table(test$class, pred,dnn=c("true","pred"))[2,1])
...
write.table(t(as.matrix(result)), file="E:/zwh/education/graduated/1导师布置/小论文5（GTB）/code/code2/comparative methods/Logisticresult.csv",col.names=F,row.names=F,sep=',',append=T) 