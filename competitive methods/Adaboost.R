...
for(j in mnum){ 
  ...
  for (i in 1:k){
      ...
      adamodel <- boosting(class~.,data = train,mfinal=j)
      pre_decisiontree_ada <- predict(adamodel,newdata = test)$class
      pre_decisiontree_ada <- pre_decisiontree_ada$class
      table(test$class,pre_decisiontree_ada,dnn=c("真实值","预测值"))
      decisiontree_roc_ada <- roc(test$class,as.numeric(pre_decisiontree_ada))
      auc <- decisiontree_roc_ada[["auc"]][1]
      TP<-as.numeric(table(test$class, pre_decisiontree_ada,dnn=c("true","pred"))[2,2])
      TN<-as.numeric(table(test$class, pre_decisiontree_ada,dnn=c("true","pred"))[1,1])
      FP<-as.numeric(table(test$class, pre_decisiontree_ada,dnn=c("true","pred"))[1,2])
      FN<-as.numeric(table(test$class, pre_decisiontree_ada,dnn=c("true","pred"))[2,1])
    ...
    #write.table(t(as.matrix(result)), file="E:/zwh/education/graduated/1导师布置/小论文5（GTB）/code/code2/comparative methods/Adaresult.csv",col.names=F,row.names=F,sep=',',append=T)
}