...
library(kknn)
library(ROCR)
#knum <- seq(3,9,by = 2) tuned
distancenum <- seq(6,10,by = 1)
for(j in distancenum){  
  ...
  for (i in 1:k){
    ...
    knn_model <-kknn(class~.,train,test,k=5,distance = j)
    prediction <- fitted(knn_model)
    #prediction <- predict(knn_model,test[,!names(test) %in% c("class")],probability=TRUE)
  ...
  write.table(t(as.matrix(result)), file="E:/zwh/education/graduated/1导师布置/小论文5（GTB）/code/code2/comparative methods/knnresult.csv",col.names=F,row.names=F,sep=',',append=T)
  #write.table(t(as.matrix(result)), file="/root/zwhtest/GBT/RFresult.csv",col.names=F,row.names=F,sep=',',append=T) 
  #write.csv(t(as.matrix(result)),file="E:/zwh/education/graduated/1导师布置/小论文5（GTB）/code/code2/comparative methods/knnresult.csv")
