...
for(j in cost){
  ...
  for (i in 1:k){
    ...
    svmmodel <- svm(class~.,data = train,kernel = "radial",cost = j,gamma = 1/ncol(train),probability = TRUE)
    prediction <- predict(svmmodel,test[,!names(test) %in% c("class")],probability=TRUE)
  ...
  #write.table(t(as.matrix(result)), file="/root/zwhtest/GBT/SVMresult.csv",col.names=F,row.names=F,sep=',',append=T) 
  write.table(t(as.matrix(result)), file="E:/zwh/education/graduated/1导师布置/小论文5（GTB）/code/code2/comparative methods/SVMresult.csv",col.names=F,row.names=F,sep=',',append=T)
}